...
void render_dungeon(dungeon_space_t dungeon[][DUNGEON_X],
  ...
  if (mh->size == 0)
    ...
    for (y = 0; y < DUNGEON_Y; y++) {
      for (x = 0; x < DUNGEON_X; x++) {
        if(fog){
          if (character_map[y][x] && y >= pc->location.ypos - PC_RADIUS && y <= pc->location.ypos + PC_RADIUS && x >= pc->location.xpos - PC_RADIUS && x <= pc->location.xpos + PC_RADIUS) {
            i = has_characteristic(character_map[y][x]->characteristic, PC) ?     CYAN : RED;
